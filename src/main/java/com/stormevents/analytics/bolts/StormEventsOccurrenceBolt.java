...
public class StormEventsOccurrenceBolt  extends BaseRichBolt {
  /**
   * 
   */
  private static final long   serialVersionUID = 1L;
  private static final Logger LOG              = Logger.getLogger(StormEventsOccurrenceBolt.class);
  
  private Map<String, BigInteger> counterMap          = Maps.newHashMap();
  ...
  public void declareOutputFields(OutputFieldsDeclarer declarer) {
    declarer.declare(new Fields("key", "num_occ", "year", "month", "date", "event_type"));
  }
  ...
  public void execute(Tuple input) {
    ...
    if(counterMap.containsKey(key)){
      counterMap.get(key).add(BigInteger.ONE);
    }else{
      counterMap.put(key, BigInteger.ONE);
    }
    outputMap.put(key,new Values(key, counterMap.get(key).intValue(), year, month, calendar.getTime(), event_type));
  ...
  class CollecterTimer extends TimerTask {
    ...
    public void run() {
      synchronized (this) {
        LOG.info("emiting occurence damage to sink... ");
